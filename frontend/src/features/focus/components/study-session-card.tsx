...
import { CalendarDays, CheckCircle2, CircleDot, Timer, Zap } from "lucide-react";
...
export function StudySessionCard({ active, copy, onComplete, onSelect, session }: StudySessionCardProps) {
    ...
    >
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <Button onClick={() => onSelect(session.id)} size="sm" variant={active ? "primary" : "secondary"}>
            {active ? copy.actions.start : copy.actions.select}
          </Button>
          <Button disabled={isCompleted} onClick={() => onComplete(session.id)} size="sm" variant="ghost">
      ...
      <div className="mt-4 grid gap-2 text-sm text-[var(--landing-text-muted)] sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Meta icon={Timer} label={copy.list.estimated(session.estimatedMinutes)} value={copy.list.focusLogged(session.completedFocusMinutes)} />
