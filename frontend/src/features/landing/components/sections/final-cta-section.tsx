    ...
    <section className="relative py-14 sm:py-22">
      ...
      <div className="landing-reveal" style={revealStyle(80)}>
        <div
          ...
          className="landing-card-strong overflow-hidden rounded-[1.4rem] border border-(--landing-border) bg-[var(--landing-panel)] px-4 py-6 shadow-[var(--landing-shadow)] sm:rounded-[2.5rem] sm:px-10 sm:py-12"
        >
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_15%_15%,var(--landing-glow),transparent_34%),radial-gradient(circle_at_86%_20%,var(--landing-cta-veil),transparent_22%)] opacity-40 sm:opacity-70" />
          <div className="relative flex flex-col gap-5 sm:gap-8 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-2xl space-y-3 sm:space-y-4">
              ...
              <h2 className="text-2xl font-semibold tracking-[-0.05em] text-(--landing-text) sm:text-[2.7rem]">
                {copy.title}
              </h2>
              <p className="text-sm leading-6 text-(--landing-text-muted) sm:text-base sm:leading-7">
                {copy.body}
              </p>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              <Link
                className="inline-flex h-12 w-full items-center justify-center rounded-full border border-(--landing-border) bg-(--landing-button-secondary) px-5 text-sm font-medium text-(--landing-text-soft) transition duration-300 hover:-translate-y-0.5 hover:bg-(--landing-button-secondary-hover) hover:text-(--landing-text) sm:w-auto"
                href="/login"
              >
                {copy.secondaryCta}
              </Link>
